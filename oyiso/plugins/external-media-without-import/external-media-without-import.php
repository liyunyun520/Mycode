...
function init_emwi() {
	$style = 'emwi-css';
	$css_file = file_uri().'/plugins/external-media-without-import/external-media-without-import.css';
	wp_register_style( $style, $css_file );
	wp_enqueue_style( $style );
	$script = 'emwi-js';
	$js_file = file_uri().'/plugins/external-media-without-import/external-media-without-import.js';
...
?>
	<div id="emwi-media-new-panel" <?php if ( $is_in_upload_ui ) : ?>style="display: none"<?php endif; ?>>
		<label id="emwi-urls-label"><?php echo __('从URL添加媒体'); ?></label>
		<textarea id="emwi-urls" rows="<?php echo $is_in_upload_ui ? 3 : 10 ?>" name="urls" required placeholder="<?php echo __("请填写媒体外链URL.\n支持多个URL，一行一个（最后一行请勿留空格）");?>" value="<?php if ( isset( $_GET['urls'] ) ) echo esc_url( $_GET['urls'] ); ?>"></textarea>
		...
		<div>
			...
			<?php echo _('请手动填写以下属性。如果宽度/高度字段留空（或0），将尝试自动解析它们。'); ?>
		</div>
...
function wp_ajax_add_external_media_without_import() {
	...
	foreach ( $attachment_ids as $attachment_id ) {
		if ( $attachment = wp_prepare_attachment_for_js( $attachment_id ) ) {
			...
			$error = "有一个附件已成功插入媒体库，但未能从数据库中检索到要显示在页面上的附件。";
		}
	}
	$info['attachments'] = $attachments;
	if ( isset( $error ) ) {
		$info['error'] = isset( $info['error'] ) ? $info['error'] . "\n另一个错误也发生了。 " . $error : $error;
	}
